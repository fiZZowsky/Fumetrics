...
export function ScannerModal({ groupedAgents, onClose, onToggleService }: ScannerModalProps) {
  ...
  const handleScan = async (overrideIp = ip, overridePort = port) => {
    ...
    try {
      ...
      alert('Nie udało się połączyć z agentem.');
      setServices([]);
        ...
        <div className="flex flex-col md:flex-row gap-6 flex-1 min-h-0">
          ...
          <div className="w-full md:w-2/3 flex flex-col min-h-87.5">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4 bg-slate-950 p-4 rounded-xl border border-slate-800 shrink-0">
              <div><label className="text-[11px] font-semibold text-slate-400 uppercase">Nazwa Maszyny</label><input type="text" value={machineName} onChange={e => setMachineName(e.target.value)} className="w-full mt-1 bg-slate-900 text-xs text-slate-200 rounded-lg px-3 py-2 border border-slate-700 outline-none" /></div>
              <div><label className="text-[11px] font-semibold text-slate-400 uppercase">Adres IP</label><input type="text" value={ip} onChange={e => setIp(e.target.value)} className="w-full mt-1 bg-slate-900 text-xs text-slate-200 rounded-lg px-3 py-2 border border-slate-700 outline-none" /></div>
              <div><label className="text-[11px] font-semibold text-slate-400 uppercase">Port Agenta</label>
                <div className="flex gap-2 mt-1">
                  <input type="text" value={port} onChange={e => setPort(e.target.value)} className="w-full bg-slate-900 text-xs text-slate-200 rounded-lg px-3 py-2 border border-slate-700 outline-none" />
                  <button onClick={toggleFavorite} className={`p-2 rounded-lg border ${isCurrentSaved ? 'bg-rose-500/10 border-rose-500/30 text-rose-400' : 'bg-slate-900 border-slate-700 text-slate-400'}`}><Heart className="w-4 h-4" fill={isCurrentSaved ? "currentColor" : "none"} /></button>
                  <button onClick={() => handleScan()} className="bg-cyan-600 hover:bg-cyan-500 text-white text-xs px-4 py-2 rounded-lg">{loading ? '...' : 'Szukaj'}</button>
                </div>
              </div>
            </div>
            ...
            {services.length > 0 && (
              <div className="mb-3 relative shrink-0">
                ...
                <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Filtruj..." className="w-full bg-slate-950 text-xs text-slate-200 rounded-lg pl-9 pr-3 py-2.5 border border-slate-800 outline-none" />
              </div>
            )}

            <div className="flex-1 overflow-y-auto border border-slate-800 rounded-xl bg-slate-950 p-2 custom-scrollbar">
               {services.length === 0 ? <div className="text-center py-12 text-slate-500 text-xs">Brak danych.</div> : (
                      ...
                      {filteredServices.map(srv => {
                        ...
                        return (
                          <tr key={srv.serviceName} className="border-b border-slate-900 hover:bg-slate-900/50">
                            <td className="p-3"><button onClick={() => onToggleService(machineName, srv.serviceName, isMonitored)} className="text-cyan-400">{isMonitored ? <CheckSquare className="w-4 h-4 text-emerald-400" /> : <Square className="w-4 h-4 text-slate-600" />}</button></td>
                            <td className="p-3 font-semibold text-slate-200">{srv.serviceName} <span className="font-normal text-slate-500 block">{srv.displayName}</span></td>
