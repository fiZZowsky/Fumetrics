...
export function AlertsModal({ uniqueMachines, groupedAgents, onClose }: AlertsModalProps) {
  ...
  // Domyślne wartości: np. Czekaj 3 min, przypominaj co 60 min.
  const [newRule, setNewRule] = useState<AlertRule>({ machineName: '', serviceName: '', metric: 'State', threshold: 'STOPPED', email: '', delayMinutes: 3, repeatMinutes: 60 });
  ...
  const cancelEdit = () => { setNewRule({ machineName: '', serviceName: '', metric: 'State', threshold: 'STOPPED', email: '', delayMinutes: 3, repeatMinutes: 60 }); setEditingId(null); };
    ...
    <div className="fixed inset-0 bg-slate-900/20 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#121A2F] border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-5xl p-8 shadow-2xl flex flex-col max-h-[85vh] transition-colors">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-extrabold flex items-center gap-3 text-slate-800 dark:text-slate-100"><div className="p-2 bg-amber-100 dark:bg-amber-500/20 rounded-xl"><Bell className="w-6 h-6 text-amber-600 dark:text-amber-400" /></div> Reguły Powiadomień z Maszyną Stanów</h2>
        ...
        <div className={`border p-5 rounded-2xl mb-8 grid grid-cols-1 md:grid-cols-4 gap-5 transition-colors shadow-inner ${editingId ? 'bg-indigo-50/50 dark:bg-indigo-950/20 border-indigo-200 dark:border-indigo-800/50' : 'bg-slate-50 dark:bg-[#0A0F1C] border-slate-200 dark:border-slate-800'}`}>
           ...
           <div className="col-span-1">
             <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Cel</label>
           ...
           <div className="col-span-2">
             <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Email odbiorcy</label>
           ...
           <div className="col-span-1">
             <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider" title="Opóźnienie wysyłki maila, dające usłudze czas na powrót do normy.">Czas zwłoki (min)</label>
             <input type="number" value={newRule.delayMinutes} onChange={e => setNewRule({...newRule, delayMinutes: parseInt(e.target.value) || 0})} min="0" className="w-full mt-1.5 bg-white dark:bg-slate-900 text-sm font-semibold text-slate-700 dark:text-slate-200 p-2.5 border border-slate-300 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 shadow-sm" />
           </div>

           <div className="col-span-1">
             <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider" title="Jeśli wpiszesz 0, dostaniesz tylko jeden alert bez przypomnień.">Przypomnienie (min)</label>
             <input type="number" value={newRule.repeatMinutes} onChange={e => setNewRule({...newRule, repeatMinutes: parseInt(e.target.value) || 0})} min="0" className="w-full mt-1.5 bg-white dark:bg-slate-900 text-sm font-semibold text-slate-700 dark:text-slate-200 p-2.5 border border-slate-300 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 shadow-sm" />
           </div>

           <div className="col-span-4 flex items-end gap-3 mt-2 border-t border-slate-200 dark:border-slate-800 pt-4">
             <button onClick={saveRule} className={`flex-1 text-white text-sm font-bold p-3 rounded-xl transition-colors shadow-md ${editingId ? 'bg-indigo-600 hover:bg-indigo-500' : 'bg-amber-500 hover:bg-amber-400'}`}>
               {editingId ? 'Zapisz Zmiany' : 'Utwórz Regułę Maszyny Stanów'}
             </button>
             {editingId && <button onClick={cancelEdit} className="w-1/4 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-white text-sm font-bold p-3 rounded-xl transition-colors">Anuluj</button>}
           </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar border border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50 dark:bg-[#0A0F1C] shadow-inner">
           <table className="w-full text-left border-collapse">
              <thead className="bg-white/90 dark:bg-slate-900/90 text-[10px] font-extrabold uppercase text-slate-500 sticky top-0 backdrop-blur-sm z-10 border-b border-slate-200 dark:border-slate-800">
                <tr><th className="p-4">Cel</th><th className="p-4">Warunek</th><th className="p-4 max-w-50">Odbiorcy</th><th className="p-4">Zwłoka / Przypom.</th><th className="p-4 text-right">Akcje</th></tr>
              </thead>
              <tbody className="text-sm font-mono text-slate-700 dark:text-slate-300">
                {rules.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-slate-500 font-sans">Brak skonfigurowanych reguł.</td></tr>}
                {rules.map(rule => (
                   <tr key={rule.id} className={`border-b border-slate-200 dark:border-slate-800 transition-colors ${editingId === rule.id ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'hover:bg-slate-100 dark:hover:bg-slate-800/50'}`}>
                      ...
                      <td className="p-4 text-slate-500 text-xs">Czeka: <span className="font-bold">{rule.delayMinutes}m</span> | Powtórz: <span className="font-bold">{rule.repeatMinutes}m</span></td>
                      <td className="p-4 text-right flex justify-end gap-2">
                        <button onClick={() => editRule(rule)} className="text-slate-400 hover:text-indigo-500 p-2 bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 transition-colors"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => removeRule(rule.id!)} className="text-slate-400 hover:text-rose-500 p-2 bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </td>
                   </tr>
                ))}
              </tbody>
